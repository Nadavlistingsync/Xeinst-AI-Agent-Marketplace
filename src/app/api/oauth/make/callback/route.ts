...
export async function GET(request: NextRequest) {
  try {
    ...
    const tokenResponse = await fetch('https://www.make.com/oauth/token', {
      method: 'POST',
      headers: {
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri: `${process.env.NEXTAUTH_URL}/api/oauth/make/callback`,
      }),
    });
    ...
    // Get user info from Make.com
    const userResponse = await fetch('https://www.make.com/api/v2/users/me', {
      headers: {
    ...
    return NextResponse.redirect(
      `${redirectUrl}?success=make_connected&agentId=${agentId}`
    );

  } catch (error) {
    console.error('Make.com OAuth callback error:', error);
    return NextResponse.redirect(
